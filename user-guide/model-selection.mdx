...
**Recommended: `gemini-3.1-flash-image`** (Gemini 3.1 Flash Image)
...
**Recommended: `anthropic.claude-sonnet-5`** (Claude Sonnet 5)
...
**When to upgrade:** `anthropic.claude-opus-5` (Claude Opus 5) for the highest-stakes content — investor-facing materials, long-form thought leadership, or anything where the first draft needs to be very close to publishable. Opus 5 keeps Opus 4.8's \$5/\$25 per-million-token price while improving agentic knowledge-work results ([Artificial Analysis](https://artificialanalysis.ai/articles/claude-opus-5-leader-agentic-knowledge-work)).

### Handwritten text extraction and document OCR

**Recommended: `anthropic.claude-sonnet-5` or `gpt-5.5`**
...
**Recommended: `gemini-3.7-flash`** (Gemini 3.7 Flash)
...
**Recommended: `anthropic.claude-sonnet-5` at minimum; `anthropic.claude-opus-5` strongly preferred for high-value batches**
...
For the Field Event Luma Engagement work plan and batch CRM enrichment, use `anthropic.claude-opus-5`. The per-lead cost increase is small relative to the value of correctly qualifying and personalizing outreach to high-intent event leads.
...
**Recommended: `openrouter/x-ai/grok-4.6`** (Grok 4.6)
...
**Recommended: `gemini-3.7-flash`** or **`anthropic.claude-sonnet-5`**
...
| **Marketing copy** | `anthropic.claude-sonnet-5` | Current Sonnet quality at Sonnet pricing | Opus 5 for investor/long-form |
| **Handwriting OCR** | `anthropic.claude-sonnet-5` or `gpt-5.5` | Frontier vision and document reasoning | Use the model selected for the surrounding workflow |
...
| **Lead enrichment** | `anthropic.claude-opus-5` | Deep research, accurate ICP scoring, quality outreach | Sonnet 5 for higher-throughput runs |
| **Coding / skills** | `anthropic.claude-sonnet-5` | Current Sonnet for coding and agentic work | DeepSeek V4 for batch/cost |
| **X posts / tweets** | `openrouter/x-ai/grok-4.6` | X-specific writing and ranking context | Sonnet 5 for non-X long-form writing |
...
| **Mid-range** | `gemini-3.7-flash`, `anthropic.claude-sonnet-5`, `gpt-5.5`, `openrouter/deepseek/deepseek-v4-pro` | Day-to-day work — content, campaigns, CRM, research |
| **Premium** | `anthropic.claude-opus-5`, `openrouter/openai/gpt-5.6-sol` | Lead enrichment, complex multi-step work plans, frontier reasoning |
...
- **Coding:** [AWS](https://aws.amazon.com/blogs/machine-learning/introducing-claude-sonnet-5-on-aws-anthropics-most-capable-sonnet-model/) documents Sonnet 5 as its most capable Sonnet model for coding and agentic work. Use the product's current model ID, not retired 4.x IDs.
