...
**Recommended: Claude Sonnet 5 or GPT-5.6 Sol**

Handwriting OCR benefits from a model that can accept image inputs and reason over the surrounding document structure. Results depend on the handwriting, scan quality, and document layout, so validate the model on representative forms before using it for high-stakes extraction.

Claude Sonnet 5 is the primary recommendation for dense, structured documents — contracts, multi-table pages, and technical reports — where the model must interpret as well as transcribe. GPT-5.6 Sol is Emma's current premium GPT option and a suitable alternative when it is already selected for the surrounding workflow.
...
| **Handwriting OCR** | Claude Sonnet 5 or GPT-5.6 Sol | Frontier vision and document reasoning | Use the model selected for the surrounding workflow |
