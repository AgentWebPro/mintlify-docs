...
description: "Use Emma in Claude.ai, Claude Code, Codex, or OpenClaw."
icon: "plug"
---
...
<Visibility for="agents">
  ...
  * **Auth (Claude Code, Codex, OpenClaw)**: API key via `Authorization` header. Key format: `aw_<hex>`. Issue keys at `https://app.agentweb.pro` under Settings → API Keys.
...
For Claude Code, Codex, and OpenClaw, generate an API key:
...
<Steps>
  <Step title="Add Emma as a connector">
    <Tabs>
      ...
      <Tab title="Codex">
        ...
        ```bash
        export EMMA_API_KEY=aw_YOUR_API_KEY_HERE
        ```

        Then add Emma to Codex:

        ```bash
        codex mcp add emma \
          --url https://api.agentweb.pro/mcp \
          --bearer-token-env-var EMMA_API_KEY
        ```

        | Argument | Description |
        | --- | --- |
        | `emma` | A local alias for the connection. Any name works. |
        | `--url` | Emma's Streamable HTTP MCP endpoint. |
        | `--bearer-token-env-var` | The environment variable Codex reads and sends as the `Authorization` bearer token. |

        Run `codex mcp list` to verify the connection. The `emma` server should appear.

        You can also run `/mcp` in the Codex TUI to inspect active MCP servers.

        <Warning>
          Do not hardcode your API key in `~/.codex/config.toml`. Keep it in an environment variable or secret manager. If a key leaks, revoke it in **Settings → API Keys**.
        </Warning>
      </Tab>
      <Tab title="OpenClaw">
  ...
  <Step title="Use Emma in any chat">
    ...
    - _"Emma, what's in my brand guidelines?"_
    - _"Pull my latest Meta campaign performance and tell me what to cut."_
    - _"Draft three LinkedIn posts in my founder voice about \[topic\]."_
    - _"Search my CRM for leads who haven't replied in two weeks."_
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Codex: tools don't appear">
    ...
    2. Run `codex mcp get emma` to confirm the endpoint is `https://api.agentweb.pro/mcp`.
    3. Confirm `EMMA_API_KEY` is available in the shell or environment where Codex starts.
    4. Restart Codex. MCP servers load when a session starts.
  </Accordion>
  ...
  <Accordion title="Revoke a leaked API key">
    ...
    4. Update Claude Code (`claude mcp remove emma`, then add again), Codex (`codex mcp remove emma`, then add again), or OpenClaw with the new key.
