...
AgentWeb has both read and write access to ClickUp. Emma can create new documents in a workspace and create new pages inside an existing document. It cannot edit existing pages, and it cannot delete documents, pages, or any other content. Writes use the same Personal API Token as reads — connecting ClickUp grants both, and there is no read-only option.
...
| **Create** | Create a new document in a workspace, or a new page inside an existing document. |
...
Disconnecting only removes the stored token. Any documents or pages Emma already created in your workspace stay where they are — AgentWeb has no way to delete them.
